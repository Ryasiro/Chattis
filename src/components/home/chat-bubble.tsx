...
const ChatBubble = ({me,message,previousMessage}:ChatBubbleProps) => {
	...
	const renderMessageContent = () => {
		switch (message.messageType) {
			...
			case "video":
				return <VideoMessage message={message} handleClick={() => setOpen(true)} />;
			default:
				return null;
		}
	};
			...
			<DateIndicator message={message} previousMessage={previousMessage} />
				<div className='flex gap-1 w-2/3'>
					<ChatBubbleAvatar isGroup={isGroup} isMember={isMember} message={message}  />
						<div className={`flex  z-20 max-w-fit px-2 pt-2 rounded-md shadow-md relative ${bgClass}`}>
	...
	<>
		<DateIndicator message={message} previousMessage={previousMessage} />
			<div className='flex gap-1 w-2/3 ml-auto'>
				<div className={`flex z-20 max-w-fit px-2 pt-2 rounded-md shadow-md ml-auto relative ${bgClass}`}>
...
const VideoMessage = ({ message,handleClick }: { message: IMessage,handleClick: () => void }) => {
	return <ReactPlayer url={message.content} width='350px' height='250px' controls={true} light={true} />;
};
